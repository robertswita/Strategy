    ...
    internal class TDispelMap: TMap
    {
        ...
        public override void ReadMap(string filename)
        {
            ...
            foreach (var sprite in Game.Sprites)
                ...
                {
                    var npc = (TNpc)sprite;
                    npc.ActPos = npc.Path[0].Position;
                }
        ...
        public void LoadExtras(string filename)
        {
            ...
            using (var reader = new BinaryReader(File.OpenRead(mapRefPath)))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    pos = cell.Position;
                    element.X = (int)pos.X + TCell.Width;
                    element.Y = (int)pos.Y + TCell.Height;
            ...
            using (var reader = new BinaryReader(File.OpenRead(mapRefPath)))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    for (int j = 0; j < pathPts.Count; j++)
                    {
                        ...
                        {
                            var pos = TransformGrid(pts[j].X, pts[j].Y);
                            //npc.Cell = Game.Cells[(int)pos.Y, (int)pos.X];
                            //pos = npc.Cell.Position + new Vector2(TGame.TileWidth, TGame.TileHeight);
                            //pos.X = (int)((pos.X + 1) * TGame.TileWidth / 2);
                            //pos.Y = (int)(pos.Y * TGame.TileHeight / 2);
                            pathPts[j] = pos;// pts[j];// YUnHex(pos);
                        }
                    }
                    //npc.X = (int)((pathPts[0].X + 1) * TGame.TileWidth / 2);
                    //npc.Y = (int)(pathPts[0].Y * TGame.TileHeight);

                    //if (pathPts[0].X > 0)
                    {
                        //npc.Cell = Game.Cells[(int)pathPts[0].Y, (int)pathPts[0].X];
                        npc.Cell = Game.Cells[0, 0];
                        //npc.X = (int)npc.Cell.Position.X + TGame.TileWidth;
                        //npc.Y = (int)npc.Cell.Position.Y + TGame.TileHeight;
                        npc.CalcPath(pathPts);
                    }
                    //if (npc.PathPts.Count > 1)
                    reader.ReadInt32();
                    reader.ReadInt32();
                    reader.ReadInt32();
                    reader.ReadInt32();
