    ...
    class TDiabloAnimationLayer
    {
        ...
        public void ReadDcc()
        {
            ...
            for (int d = 0; d < directionsCount; d++)
            {
                ...
                var bs = new TBitStream(reader.ReadBytes(streamSize));
                bs.Size = streamSize;
                ...
                var copyBlocksStream = new TBitStream(bs.Buffer);
                if (hasStillBlocks)
                    copyBlocksStream.Size = bs.Read(20);
                var codesMaskStream = new TBitStream(bs.Buffer);
                codesMaskStream.Size = bs.Read(20);
                var rawCodesEnabledStream = new TBitStream(bs.Buffer);
                var rawCodesStream = new TBitStream(bs.Buffer);
                if (hasRawPixels)
                {
                    rawCodesEnabledStream.Size = bs.Read(20);
                    rawCodesStream.Size = bs.Read(20);
                }
                var codesStream = new TBitStream(bs.Buffer);
                ...
                var boundBlocks = new TMacroBlock[(bounds.Height + 3) >> 2, (bounds.Width + 3) >> 2];
                var framesBlocks = new TMacroBlock[dir.Length][,];
                for (var f = 0; f < dir.Length; f++)
                {
                    var frame = dir[f];
        ...
        TMacroBlock[,] CreateMacroblocks(TFrame frame)
        {
            ...
            var blocks = new TMacroBlock[blocksYCount, blocksXCount];
            var currHeight = 0;
                        ...
                        do
                        {
                            ...
                            else
                            {
                                for (int x = segBegin; x < segBegin + segSize; x++)
                                    pixmap[x, y] = Palette[reader.ReadByte()];
                                segBegin += segSize;
                            }
                        } while (true);
                    }
