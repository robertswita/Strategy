    ...
    class TDiabloTile : TTile
    {
        ...
        byte[] EncodeIsometric(Bitmap bmp)
        {
            ...
            for (int y = 0; y < pixmap.Height; y++)
            {
                ...
                for (int x = pixmap.Width / 2 - r; x < pixmap.Width / 2 + r; x++)
                {
                    var colorIdx = TDiabloMap.Palette.GetNearestColor(pixmap[x, y]);
                    IsEmpty &= colorIdx == 0;
                    pixels[pos] = colorIdx;
                    pos++;
                }
        ...
        public override bool IsEqual(TTile other)
        {
            var otherTile = other as TDiabloTile;
            for (int i = 0; i < EncodedPixels.Length; i++)
                if (EncodedPixels[i] != otherTile.EncodedPixels[i])
                    return false;
            return true;
        }
