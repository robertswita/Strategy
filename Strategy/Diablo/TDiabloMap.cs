...
namespace Strategy
{
    class TDiabloMap : TMap
    {
        ...
        public void WriteTileSet(string filename, string ext)
        {
            ...
            writer.Write(276);
            var tileHeaderPos = 276 + Walls.Count * 96;
            for (int i = 0; i < Walls.Count; i++)
            {
                var wall = (TDiabloWall)Walls[i];
                wall.WriteHeader(writer);
                writer.Write(tileHeaderPos);
                var tileDataSize = 20 * wall.Tiles.Count;
                for (int j = 0; j < wall.Tiles.Count; j++)
                {
                    var tile = (TDiabloTile)wall.Tiles[j];
                    tile.Encode();
                    tileDataSize += tile.EncodedPixels.Length;
                }
                writer.Write(tileDataSize);
                tileHeaderPos += tileDataSize;
                writer.Write(wall.Tiles.Count);
        ...
        {
            Width = (WorldWidth + WorldHeight) / 5;
            Height = Width / 2;
            ...
            //    }
            for (int y = 0; y < WorldHeight / 5; y++)
                for (int x = 0; x < WorldWidth / 5; x++)
                {
        ...
        TCell[,] TransformToHexMapping()
        {
            ...
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    var pos = Map2WorldTransform(5 * x, 5 * y);
                    if (pos.Y >= 0 && pos.Y < WorldHeight && pos.X >= 0 && pos.X < WorldWidth)
                    {
                        var cell = Cells[y, x];
                        //cell.X = (int)pos.X;
        ...
        public void MapTileSet(List<TTile> tiles)
        {
            ...
            for (int idx = 0; idx < Walls.Count; idx++)
            {
                ...
                //var v = World2ViewTransform(blockTile.X, blockTile.Y);
            ...
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (selection != null)
                    {
                        ...
                        wall.Order = layerIdx * Width * Height + y * Width + x;
                        var pos = World2ViewTransform(5 * x, 5 * y);
                        wall.X = (int)pos.X - 5 * TDiabloTile.Width / 2;
                        wall.Y = (int)pos.Y - 5 * TDiabloTile.Height;
        ...
        void WriteWallsLayer(BinaryWriter writer, List<TDiabloWall>[,,] blockTiles, int layerIdx)
        {
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (cell.Piece != null)
                    {
                        var wall = (TDiabloWall)Walls[cell.Piece.Index];
                        var selection = blockTiles[layerIdx + 4, wall.Style, wall.Seq];
                        if (selection != null)
                            tileInfo = wall.Style << 20 | wall.Seq << 8 | 0x80 | wall.Rarity;
                    }
        ...
        void ReadFloors(BinaryReader reader, List<TDiabloWall>[,,] blockTiles)
        {
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (selection != null)
                    {
                        wall = selection[TGame.Random.Next(selection.Count)];
                        var floor = Floors[wall.Index];
                        if (floor == null)
                        {
                            floor = new TTile();
                            floor.Index = wall.Index;
                            var floorImage = new Bitmap(wall.Width, 80);
                            ...
                            for (int u = 0; u < 5; u++)
                                for (int v = 0; v < 5; v++)
                                {
                                    ...
                                    var tileIdx = u * 5 + v;
                                    if (tileIdx < wall.Tiles.Count)
                                    {
                                        var tile = wall.Tiles[tileIdx];
                                        //cell.Floor = wall.Tiles[tileIdx];
                                        //cell.Floor.Index = wall.Index;
                                        gc.DrawImage(tile.Image, tile.X, tile.Y);
                                    }
                                }
                            floor.Image = floorImage;
                            Floors[floor.Index] = floor;
                            //Floors.Add(floor);
                        }
        ...
        void WriteFloors(BinaryWriter writer)
        {
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (cell.Floor != null)
                    {
                        var floor = (TDiabloWall)Walls[cell.Floor.Index];
                        tileInfo = floor.Style << 20 | floor.Seq << 8 | 0xc0 | floor.Rarity;
                    }
                    writer.Write(tileInfo);
                }
        }
        ...
        void WriteCells(BinaryWriter writer)
        {
            ...
            var blockTiles = new List<TDiabloWall>[8, 64, 64];
            foreach (var blockTile in Walls)
            {
                var dbTile = (TDiabloWall)blockTile;
                if (blockTiles[(int)dbTile.Type, dbTile.Style, dbTile.Seq] == null)
            ...
            for (int i = 0; i < WallsLayersCount; i++)
                WriteWallsLayer(writer, blockTiles, i);
            for (int i = 0; i < floorsLayersCount; i++)
                WriteFloors(writer);
            WriteShadows(writer);
            if (SubstitutionType == 1 || SubstitutionType == 2)
