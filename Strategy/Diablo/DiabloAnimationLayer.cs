...
{
    internal class DiabloAnimationLayer
    {
        class MacroBlock
        {
        ...
        public void ReadDcc()
        {
            ...
            for (int d = 0; d < directionsCount; d++)
            {
                ...
                codesMaskStream.Position = stillBlockStream.Position + stillBlockStream.Size;
                ...
                var boundBlocksPalette = new int[(bounds.Height + 3) >> 2, (bounds.Width + 3) >> 2][];
                ...
                for (var f = 0; f < frames.Length; f++)
                {
                    ...
                    {
                        var codesMask = 0;
                        var blockPalette = boundBlocksPalette[block.PosY >> 2, block.PosX >> 2];
                        if (blockPalette == null)
                        {
                            blockPalette = new int[4];
                            boundBlocksPalette[block.PosY >> 2, block.PosX >> 2] = blockPalette;
                            codesMask = 0xF;
                        }
                        else if (stillBlockStream.Size == 0 || !stillBlockStream.ReadBool())
                            codesMask = codesMaskStream.Read(4);
                        else
                            block.IsStill = true;
                        var stackCodesCount = codesMask - (codesMask >> 1) - (codesMask >> 2) - (codesMask >> 3);
                        var stackCodes = new Stack<int>(stackCodesCount);
                        var rawCodesEnabled = false;
                        if (codesMask > 0 && rawCodesEnabledStream.Size > 0)
                            rawCodesEnabled = rawCodesEnabledStream.ReadBool();
                        var prevCode = 0;
                        for (var i = 0; i < stackCodesCount; i++)
                        {
                        ...
                        for (int i = 0; i < blockPalette.Length; i++)
                            if ((codesMask >> i & 1) != 0)
                            {
                                ...
                                blockPalette[i] = palEntries[code];
                            }
                        block.Palette = (int[])blockPalette.Clone();
                    }
                }
                    ...
                    foreach (var block in framesBlocks[f])
                    {
                        for (int y = 0; y < block.Height; y++)
                            for (int x = 0; x < block.Width; x++)
                            {
                                ...
                                {
                                    var code = codesStream.Read(block.BitCount);
                                    color = Palette[block.Palette[code]];
                                }
