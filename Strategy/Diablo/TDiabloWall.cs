...
namespace Strategy.Diablo
{
    ...
    class TDiabloWall : TWall
    {
        ...
        public TDiabloWall()
        {
            Width = 5 * TDiabloTile.Width;
            Height = -5 * TDiabloTile.Height;
        }
        //protected override int CompareWith(TSprite other)
        //{
        //    if (Type >= 16 && other is TDiabloWall)
        //    {
        //        var otherWall = (TDiabloWall)other;
        //        var lowerWall = (int)TWallType.LowerWall;
        //        if (otherWall.Type < lowerWall)
        //            return -1;
        //            //return otherWall.Type.CompareTo(Type);
        //    }
        //    //return base.CompareWith(other);
        ...
        public void WriteHeader(BinaryWriter writer, int tilesHeaderPos)
        {
            ...
            writer.Write(Rarity);
            writer.Write(Unk);
            var tilesFlags = TilesFlags == null ? new byte[25] : TilesFlags;
        ...
        public void ReadTiles(BinaryReader reader)
        {
            for (int i = 0; i < Tiles.Capacity; i++)
            {
                ...
                if (tile.HasRleFormat)
                    tileBounds.Height *= 2;
                //Bounds = i == 0 ? tileBounds: Rectangle.Union(Bounds, tileBounds);
        ...
        //public bool HasFlags(byte[] flags)
        //{
        //    if (TilesFlags != flags)
        //    {
        //        if (TilesFlags == null || flags == null) return false;
        //        for (var i = 0; i < flags.Length; i++)
        //            if (TilesFlags[i] != flags[i]) return false;
        //    }
        //    return true;
        //}
    }
