...
namespace Strategy
{
    public class TNpc : TSprite
    {
        ...
        {
            Path = new List<TCell>();
            var worldRect = new Rectangle(0, 0, Cell.Game.Map.Width, Cell.Game.Map.Height);
            if (worldRect.Contains((int)pathPts[0].X, (int)pathPts[0].Y))
                Cell = Cell.Game.Cells[(int)pathPts[0].Y, (int)pathPts[0].X];
            var startCell = Cell;
            for (int i = 1; i < pathPts.Count; i++)
            {
                if (!worldRect.Contains((int)pathPts[i].X, (int)pathPts[i].Y)) continue;
                var stopCell = Cell.Game.Cells[(int)pathPts[i].Y, (int)pathPts[i].X];
        ...
        public override void NextFrame()
        {
            ...
            if (WaitFrameNo > 0)
            {
                ...
                if ((ActPos - nextCell.Position).Length() < 1)
                {
                    ...
                    if (nextCell != Cell)
                        ViewAngle = Cell.Neighbors.IndexOf(nextCell);
                    else
                        Wait();
                }
            }
