...
namespace Strategy
{
    class TMapConverter
    {
        ...
        public void ExportFloors()
        {
            ...
            DiabloMap.Width = size;
            DiabloMap.Height = size;
                    ...
                    var duplicatePos = Game.Map.View2MapTransform(pos.X, pos.Y);
                    {
                        ...
                        if (cell.Floor != null)
                        {
                            ...
                            for (var i = 0; i < floorRef.Count; i++)
                            {
                                ...
                                {
                                    var prevFloor = DiabloMap.Walls[prevDiabloCell.Floor.Index];
                                    if (floor.IsEqual(prevFloor))
                                    {
                                        diabloCell.Floor.Index = prevFloor.Index;
                                        duplicatesCount++;
                                        floorRef.RemoveAt(i);
                                        break;
                                    }
                            ...
                            if (diabloCell.Floor.Index > 0)
                                continue;
                        }
                    }
                    DiabloMap.Walls.Add(floor);
                    diabloCell.Floor.Index = floor.Index;
                    floor.Style = DiabloMap.Walls.Count >> 6;
        ...
        {
            var size = DiabloMap.Width;
            var walls = new List<TWall>(Game.Map.Walls);
            walls.Sort();
            var wallRefs = new TDiabloWall[size * size, 4];
            for (int i = 0; i < walls.Count; i++)
            {
                ...
                var index = diabloCell.Y / 5 * size + diabloCell.X / 5;
                TDiabloWall diabloWall = null;
                var tileType = -1;
                if (diabloCell.Piece == null)
                    diabloCell.Piece = new TPiece();
                else
                {
                    diabloWall = (TDiabloWall)DiabloMap.Walls[diabloCell.Piece.Index];
                    tileType = (int)diabloWall.Type - 4;
                }
                tileType = (tileType + 1) & 3;
                if (tileType > DiabloMap.WallsLayersCount - 1)
                    DiabloMap.WallsLayersCount++;
                diabloWall = wallRefs[index, tileType];
                if (diabloWall == null)
                {
                    diabloWall = new TDiabloWall();
                    diabloWall.Type = (TWallType)(4 + tileType);
                    diabloWall.Style = index >> 6;
                    diabloWall.Seq = index & 63;
                    wallRefs[index, tileType] = diabloWall;
                    DiabloMap.Walls.Add(diabloWall);
                }
                diabloCell.Piece.Index = diabloWall.Index;
