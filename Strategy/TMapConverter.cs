...
namespace Strategy
{
    class TMapConverter
    {
        ...
        void AddVis(TCell diabloCell, TCell dispelCell)
        {
            var eventIdx = dispelCell.EventIdx;
            ...
            for (int y = 0; y < DiabloMap.Height; y++)
                for (int x = 0; x < DiabloMap.Width; x++)
                {
                    ...
                    if (IsFloorIso)
                    {
                        ...
                        floor.Bounds = new Rectangle((int)viewPos.X, (int)viewPos.Y - 80, floor.Width, 80);
                ...
                for (int v = 0; v < 5; v++)
                {
                    ...
                    if (cellPos.X >= 0 && cellPos.X < Game.Map.Width && cellPos.Y >= 0 && cellPos.Y < Game.Map.Height)
                    {
                        ...
                        if (cell.Collision)
                        {
                            ...
                            var tileBounds = new Rectangle((int)tilePos.X - 32, (int)tilePos.Y - 8, 32, 32);
        ...
        {
            if (!IsFloorIso && diabloCell.Wall.Tiles.Count == 0) return null;
            return IsFloorIso ? (TDiabloWall)diabloCell.Floor : (TDiabloWall)diabloCell.Wall.Tiles[0]; 
        }
        ...
        TDiabloWall FindFloorDuplicate(TCell diabloCell, TDiabloWall floor, List<TCell> floorRef)
        {
            for (var i = 0; i < floorRef.Count; i++)
            {
                ...
                if (prevFloor.IsEqual(floor))
                {
                    ...
                    if (prevDiabloCell.Wall.Tiles.Count == 0 || prevDiabloCell.Floor == DiabloMap.Floors[0])
