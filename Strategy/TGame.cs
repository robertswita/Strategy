...
namespace Strategy
{
    public class TGame
    {
        ...
        public List<TSprite> RoofTiles = new List<TSprite>();
        ...
        public List<Image[][][]> Animations = new List<Image[][][]>();
        ...
        public void ImportMap(string filename)
        {
            ...
            else if (ext == ".spr")
            {
                ...
                for (var i = 0; i < Animations.Count; i++)
                {
                    ...
                    {
                        var view = animation[j];
                        posX = 0;
                        for (var k = 0; k < view.Length; k++)
                        {
                            var sprite = new TSprite();
                            sprite.Images = view[k];
                            var width = sprite.Images[0].Width;
                            var height = sprite.Images[0].Height;
                            sprite.X = posX;
                            sprite.Y = posY;
                            posX += width;
