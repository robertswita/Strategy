...
namespace Strategy
{
    public class TMap
    {
        TGame game;
        Image Background;
        public virtual Image GetBackground() { return Background; }
        public TGame Game
        {
            get { return game; }
        ...
        public virtual void ReadMap(string filename)
        {
            ...
            Game.GroundTiles = new List<Image>();
            Game.TilesImages = new List<Image>();
            for (int i = 0; i < files.Length; i++)
                Game.TilesImages.Add(ReadTexture(files[i]));
        ...
        public void LoadMap(string filename)
        {
            ...
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    {
                        var tile = new TTile();
                        tile.ImageIndex = TGame.Random.Next(Game.TilesImages.Count);
                        cell.Piece = tile;
                    }
                    else
