...
namespace Strategy
{
    public class TMap
    {
        ...
        public virtual void ReadMap(string filename)
        {
            ...
            Game.BlockTiles = new List<TTile>();
            for (int i = 0; i < files.Length; i++)
            {
                var tile = new TTile();
                tile.Index = i;
                tile.Image = ReadTexture(files[i]);
                Game.BlockTiles.Add(tile);
            }
            ...
            for (int y = 0; y < Height; y++)
                for (int x = 0; x < Width; x++)
                {
                    ...
                    if (bmp.GetPixel(x, y).R == 0)
                    {
                        ...
                        //cell.Piece = tile;
                        cell.Piece = Game.BlockTiles[TGame.Random.Next(Game.BlockTiles.Count)];
                    }
                    else
