...
namespace Strategy
{
    public partial class TBoard : UserControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                var visibleRect = vp;
                visibleRect.Intersect(sprite.Bounds);
                if (visibleRect.Width == 0 || visibleRect.Height == 0)
                    continue;
            ...
            foreach (var sprite in spriteList)
            {
                if (sprite is TColumnTile)
                {
                    ...
                    gc.DrawImage(sprite.ActFrame, sprite.X, sprite.Y);
            }
            foreach (var roofTile in Game.RoofTiles)
            {
                var visibleRect = vp;
                visibleRect.Intersect(roofTile.Bounds);
                if (visibleRect.Width == 0 || visibleRect.Height == 0)
                    continue;
                gc.DrawImage(roofTile.ActFrame, roofTile.X, roofTile.Y);
            }
